...
class SimpleRetention(nn.Module):
    ...
    def forward(self, X):
        ...
        D = self._get_D(sequence_length).to(DEVICE)

        Q = (X @ self.W_Q).to(DEVICE)
        K = (X @ self.W_K).to(DEVICE)

        Q = self.xpos(Q).to(DEVICE)
        K = self.xpos(K, downscale=True).to(DEVICE)
    ...
    def forward_recurrent(self, x_n, s_n_1, n):
        ...
        Q = (x_n @ self.W_Q).to(DEVICE)
        K = (x_n @ self.W_K).to(DEVICE)

        Q = self.xpos(Q, n + 1).to(DEVICE)
        K = self.xpos(K, n + 1, downscale=True).to(DEVICE)

        V = (x_n @ self.W_V).to(DEVICE)

        # K: (batch_size, 1, hidden_size)
        # V: (batch_size, 1, v_dim)
        # s_n = gamma * s_n_1 + K^T @ V

        s_n = self.gamma * s_n_1.to(DEVICE) + (K.transpose(-1, -2) @ V)
